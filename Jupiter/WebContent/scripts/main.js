(function() {
  ...
  function register() {
    ...
    //The MD5(str) function calculates the 128-bit checksum for a given string, returned as a string of 32 hex digit 
    password = md5(username + md5(password));

    // The request parameters
    var url = './register';
    //now everything seems good and then proceed to update database
    var req = JSON.stringify({
      user_id : username,
      password : password,
  ...
  function loadWordCloud() {
      ...
      ajax(
          ...
          // failed callback
          function() {
              showErrorMessage('Cannot load your wordcloud.');
          });
  }
